...
void PlayScene::Initialize() {
	...
	//AddNewObject(TurretGroup = new Group());
	AddNewObject(BulletGroup = new Group());
	AddNewObject(BadBulletGroup = new Group());
...
void PlayScene :: change_mapstate(int mx,  int my) {
       const int x = mx / BlockSize;
	const int y = my / BlockSize;
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	if (button & 1) {
	       if (!preview)
                     return;
              if(mapState[y][x] == TILE_OCCUPIED && preview->type == 5) {
			...
			// To keep responding when paused.
			preview->Update_remove(0);
			// Remove Preview.
			preview = nullptr;
			//change_mapstate(mx,  my);
			mapState[y][x] = TILE_FLOOR;
			OnMouseMove(mx, my);
              }
		else if (mapState[y][x] != TILE_OCCUPIED && preview->type != 5) {
		       //std::cout << preview->type << std::endl;
			// Check if valid.
			if (!CheckSpaceValid(x, y)) {
				Engine::Sprite* sprite;
				GroundEffectGroup->AddNewObject(sprite = new DirtyEffect("play/target-invalid.png", 1, x * BlockSize + BlockSize / 2, y * BlockSize + BlockSize / 2));
...
void PlayScene::Hit(int amount) {
       if(lives > 0) {
              if(amount != 15) LoseLive(1);
              else LoseLive(5);
       }
       if (lives <= 0) {
		// Free resources.
...
void PlayScene::LoseLive(int lives) {
       this->lives -= lives;
       UILives->Text = std::string("Life ") + std::to_string(this->lives);
}
void PlayScene::EarnLive(int lives) {
       this->lives += lives;
       UILives->Text = std::string("Life ") + std::to_string(this->lives);
}
...
void PlayScene::ConstructUI() {
	...
	UIGroup->AddNewControlObject(btn);
		...
		, 530, 128*MapHeight, MiniNewTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	if (id == 0 && money >= WBCellTurret::Price)
       ...
       else if (id == 3 && money >= MiniNewTurret::Price)
		preview = new MiniNewTurret(0, 0);
